...
tape("render() with-scope with literal value", function(test) {
	var document = jsdom("<div data-with='{{d}}'><span>{{d}}</span></div>");
...
tape("render() with-scope with object value", function(test) {
	var document = jsdom("<div data-with='{{d.obj}}'><span>{{d.value}}</span></div>");
	var node = document.querySelector("div");
	var selection = d3.select(node);
...
tape("render() with-scope with object value within repeat", function(test) {
	var document = jsdom("<div data-repeat='{{d}}'><div data-with='{{d.obj}}'><span>{{safe:d.value}}</span></div></div>");
	var node = document.querySelector("div");
	var selection = d3.select(node);
